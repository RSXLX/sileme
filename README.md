...
| 要求           | 实现                                | 状态 |
| -------------- | ----------------------------------- | ---- |
| **链上支付**   | 支持稳定币 (USDT) 和原生 KITE 转账  | ✅   |
| **Agent 身份** | 后端 `gokite-aa-sdk` 创建 AA Wallet | ✅   |
| **权限控制**   | perTxLimit + dailyLimit 额度限制    | ✅   |
| **可复现性**   | 完整运行说明 (见下文)               | ✅   |
...
| Kite 能力      | 使用方式                                       |
| -------------- | ---------------------------------------------- |
| Agent 身份系统 | `GokiteAASDK.getAccountAddress()` 计算 AA 地址 |
| 稳定币支付接口 | ERC-20 `transferFrom` + Kite Settlement Token  |
| 支付权限控制   | 后端 `checkSpendingLimits()` 函数实时校验      |
| 官方 SDK       | `gokite-aa-sdk@1.0.14`                         |
